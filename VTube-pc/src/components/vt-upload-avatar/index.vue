  ...
  <div class="upload-box">
    ...
    >
      <template v-if="imageUrl">
        <img :src="fileUrl + imageUrl" class="upload-image" />
        <div class="upload-handle" @click.stop>
          <div v-if="!self_disabled" class="handle-icon">
            <Icon icon="material-symbols:edit-square-outline-rounded" />
            <span>编辑</span>
          </div>
          <div class="handle-icon" @click="imgViewVisible = true">
            <Icon icon="healthicons:ui-zoom-outline" />
            <span>查看</span>
          </div>
      ...
      <template v-else>
        <div class="upload-empty">
          <slot name="empty">
            <Icon icon="ic:baseline-plus" class="text-2xl" />
          </slot>
        </div>
      </template>
    </el-upload>
...
const handleHttpUpload = async (options: UploadRequestOptions) => {
  ...
  try {
    const { data } = await httpPost('/uploadFile', formData)
    imageUrl.value = data
