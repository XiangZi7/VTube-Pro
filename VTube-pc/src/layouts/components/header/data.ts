export const data = [
  { title: '主站', icon: 'fa6-brands:bilibili', router: '/' },
  { title: '番剧放送', icon: 'game-icons:tv', router: '/anime' },
  { title: '番组计划', icon: 'system-uicons:projector', router: '/projector' },
  { title: '外部番组', icon: 'system-uicons:projector', router: '/animeV1' },
  { title: '剧场动画', icon: 'mdi:theater', router: '/theatre' },
  { title: '电影', icon: 'jam:movie', router: '/movie' },
  { title: '论坛', icon: 'material-symbols:forum-outline-rounded', router: '/forum' },
]

export const UploadData = [
  { title: '视频投稿', icon: 'fluent:video-clip-24-regular', router: '/user/upload/video' },
  { title: '论坛发表', icon: 'mdi-light:forum', router: '/forum' },
]
