...
import { Role } from '@/interface/pages/role'
...
// 打开对话框
const openDialog = (title: string, row: Partial<Role> = {}) => {
  const props = {
    title,
    model: row,
    enumMap: vtTable.value?.enumMap,
    disabled: title === '查看',
    api: title === '新增' ? '/role/add' : '/role/edit',
...
// 打开授权框
const openAssignDialog = (title: string, row: Partial<Role> = {}) => {
  const props = {
    title,
    model: row,
    enumMap: vtTable.value?.enumMap,
  ...
  AssignRef.value?.openDialog(props)
}
// 删除
const deletes = async (id: number | (number | string)[]) => {
  ...
  // 发出HTTP请求
  const { code, data, message } = await httpPost('/role/deletes', ids)
        ...
        <template #action="{ row }">
          <div class="space-x-2 flex text-xl justify-around">
            ...
            <Icon
              ...
              class="cursor-pointer"
              @click="openAssignDialog('授权', row)"
            />
            <el-popconfirm
              :title="`确定要删除《${row.name}?》`"
              @confirm="deletes(row.roleId)"
            >
              <template #reference>
