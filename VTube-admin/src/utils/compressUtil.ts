...
export function PhotoCompress(minSize, maxSize) {
   ...
   * @returns {*}
   */
  PhotoCompress.prototype.dataUrlToFile = function (
    base64Codes,
  ...
  PhotoCompress.prototype.compress = function (file, callback) {
    ...
    self.imgBase64(file, function (image, canvas) {
      const base64Codes = canvas.toDataURL(file.type, nextQ) // y压缩
      const compressFile = self.dataUrlToFile(
        base64Codes,
        file.name.split('.')[0]
      ) // 转成file文件
      const compressFileSize = compressFile.size // 压缩后文件大小 k
